...
export class AlgoliaService {
  ...
  static async searchStrategies(
    ...
    try {
      ...
      console.log('Algolia search result hits:', result.hits?.length || 0);
  ...
  static async getTrendingStrategies(subcategoryId: string, hitsPerPage: number = 10): Promise<SearchResponse> {
    const searchParams = {
      ...
      query: '',
      hitsPerPage,
      filters: `tags:"${subcategoryId}"`,
      // Sort by recent activity and rebalance frequency
      sort: ['CountLast7Days:desc', 'averageRebalanceActivity:desc']
    };
  ...
  static convertToInvestmentProduct(strategy: AlgoliaStrategy): any {
    console.log('Converting strategy:', {
      id: strategy.Id,
